...
class AirHockey:
    ...
    def __check_hit_with_player(self, pack_speed, pack_position, pack_size, player_position, player_size):
        ...
        dis = culc_distance_of_line_and_point(a, b, c, player_position)
        ...
        dis  =culc_distance_of_two_points(moved_pack_position, player_position)
        if dis <= NESS_DIS :
            # パックの位置の修正
            (intersec1, intersec2) = culc_intersection_of_circle_and_linear
            dis1 = culc_distance_of_two_points(pack_position, intersec1)
    ...
    def __check_cross_of_two_linear(self, a, b, c, d):
        ...
        outer_of_ab_and_ac= culc_outer_product(ab, ac)
        outer_of_ab_and_ad= culc_outer_product(ab, ad)
        outer_of_cd_and_ca= culc_outer_product(cd, ca)
        outer_of_cd_and_cb= culc_outer_product(cd, cb)
